...
const headers={'Cache-Control':'no-store','Content-Type':'text/html; charset=utf-8','X-Frame-Options':'DENY','Referrer-Policy':'same-origin','Content-Security-Policy':"default-src 'none'; style-src 'unsafe-inline'; form-action 'self'; frame-ancestors 'none'"};
...
export async function accessGate(request:Request,password:string|undefined):Promise<Response|null>{
 ...
 if(url.pathname==='/boo-login'&&request.method==='POST'){
  if(![url.origin,'https://boo.farimaniconsultant.com','https://boo-motion-lab.mehrdad655045.chatgpt.site'].includes(request.headers.get('Origin')??''))return new Response('Invalid origin',{status:403,headers});
